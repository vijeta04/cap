...
# By creating a Flask application instance, you can define routes,
# views, and other configuration details specific to your web application.
app = Flask(__name__)
# Bootstrap(app)
...
def predict():
    if request.method == "POST":
        ...
        features['satisfaction_level'] = int(
            features['satisfaction_level'])/100
        ...
        if features['model'] == "Decision Tree Classifier":
            ...
            model = joblib.load(
                open("ml_models/logistic_regression.sav", "rb"))
            prediction = model.predict(df)
